...
var contactUsForm = $('#addContactForm');

var validatedForm = contactUsForm.validate({
	rules: {
		name: { required: true },
	...
	messages: {
		...
		email: {
			...
			email: 'Enter valid format of email',
		},
		message: {
	...
	submitHandler: function (form) {
		showButtonLoading($("#submit"));
		var formData = new FormData(form);
		$.ajax({
			url: routeContactStore,
			method: "POST",
			dataType: "JSON",
			data: formData,
			contentType: false,
			...
			error: function (error) {
				if (error.status == 422) {
					...
					validator.setErrors(validatedForm, errors)
					$("#submit").prop("disabled", false);
				} else {
					ajaxErrorCallback(error, 'Error occurred while processing your request.');
				}
			},
			complete: function() {
				$('#reload_captcha-button').trigger('click');;
...
$('#enquiry_reload_captcha-button').on("click", function () {
	$("#captcha").val('');
	$.ajax({
		type: 'GET',
		url: '/reload-captcha',
		success: function (data) {
			$("#enquiry_captcha_image").html(data.captcha);
		}
	});
});
...
function initEvents() {
	...
	$('.menu-types').on('click', 'a', function() {
		...
		$(this).parent().find('.trip_type').val($(this).data('value'));
